...
public:
        ...
        : _runtime(runtime), _send_cb(send_cb), _builder(runtime) 
    {
        ...
        cbs.onRgaCopy = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t src_fd, uint64_t dst_fd, const RgaConfig& src_cfg, const RgaConfig& dst_cfg) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)p;
                chain.addCopy(s, d, 
        ...
        cbs.onRgaRotate = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t src_fd, uint64_t dst_fd, const RgaConfig& src_cfg, const RgaConfig& dst_cfg, int angle) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)p;
                chain.addRotate(s, d, 
                    {src_cfg.width(), src_cfg.height(), src_cfg.format(), src_cfg.x_offset(), src_cfg.y_offset()},
        ...
        cbs.onRgaFill = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t dst_fd, const RgaConfig& dst_cfg, uint32_t color) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)s; (void)p;
                chain.addFill(d, {dst_cfg.width(), dst_cfg.height(), dst_cfg.format(), dst_cfg.x_offset(), dst_cfg.y_offset()}, color);
            }, -1, dst_fd, -1, {}, dst_cfg, {});
        };
        cbs.onRgaBlend = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t src_fd, uint64_t dst_fd, const RgaConfig& src_cfg, const RgaConfig& dst_cfg, int mode) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)p;
                chain.addBlend(s, d, 
                    {src_cfg.width(), src_cfg.height(), src_cfg.format(), src_cfg.x_offset(), src_cfg.y_offset()},
        ...
        cbs.onRgaOsd = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t src_fd, uint64_t dst_fd, const RgaConfig& dst_cfg, const ImOsd& osd_cfg) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)p;
                im_osd_t params; 
        ...
        cbs.onRgaResize = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t src_fd, uint64_t dst_fd, const RgaConfig& src_cfg, const RgaConfig& dst_cfg, double fx, double fy) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)p;
                chain.addResize(s, d, 
                    {src_cfg.width(), src_cfg.height(), src_cfg.format(), src_cfg.x_offset(), src_cfg.y_offset()},
        ...
        cbs.onRgaCrop = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t src_fd, uint64_t dst_fd, const RgaConfig& src_cfg, const RgaConfig& dst_cfg) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)p;
                chain.addCrop(s, d, 
                    {src_cfg.width(), src_cfg.height(), src_cfg.format(), src_cfg.x_offset(), src_cfg.y_offset()},
        ...
        cbs.onRgaConvert = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t src_fd, uint64_t dst_fd, const RgaConfig& src_cfg, const RgaConfig& dst_cfg, int mode) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)p;
        ...
        cbs.onRgaFlip = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t src_fd, uint64_t dst_fd, const RgaConfig& src_cfg, const RgaConfig& dst_cfg, int mode) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)p;
                chain.addFlip(s, d, 
                    {src_cfg.width(), src_cfg.height(), src_cfg.format(), src_cfg.x_offset(), src_cfg.y_offset()},
        ...
        cbs.onRgaComposite = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t fg_fd, uint64_t bg_fd, uint64_t dst_fd, const RgaConfig& fg_cfg, const RgaConfig& bg_cfg, const RgaConfig& dst_cfg, int mode) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
        ...
        cbs.onRgaColorKey = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t fg_fd, uint64_t bg_fd, const RgaConfig& fg_cfg, const RgaConfig& bg_dst_cfg, const ImColorKeyRange& range, int mode) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)p;
                im_colorkey_range r;
                r.max = range.max();
        ...
        cbs.onRgaMosaic = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t dst_fd, const RgaConfig& dst_cfg, int mode) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)s; (void)p;
                chain.addMosaic(d, {dst_cfg.width(), dst_cfg.height(), dst_cfg.format(), dst_cfg.x_offset(), dst_cfg.y_offset()}, mode);
            }, -1, dst_fd, -1, {}, dst_cfg, {});
        };
        cbs.onRgaRectangle = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t dst_fd, const RgaConfig& dst_cfg, uint32_t color, int thickness) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)s; (void)p;
        ...
        cbs.onRgaFillArray = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t dst_fd, const RgaConfig& dst_cfg, const std::vector<ImRect>& rects, uint32_t color) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)s; (void)p;
        ...
        cbs.onRgaRectangleArray = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t dst_fd, const RgaConfig& dst_cfg, const std::vector<ImRect>& rects, uint32_t color, int thickness) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)s; (void)p;
                std::vector<im_rect> r;
                for (const auto& a : rects) {
        ...
        cbs.onRgaMosaicArray = [this](uint32_t usr, uint32_t part, uint32_t job_id, uint64_t dst_fd, const RgaConfig& dst_cfg, const std::vector<ImRect>& rects, int mode) {
            handleRgaGeneric(job_id, usr, part, [=](SuOS::graphics::RgaChain& chain, auto s, auto d, auto p) {
                (void)s; (void)p;
                std::vector<im_rect> r;
                for (const auto& a : rects) {
    ...
    void onMessage(uint32_t sender_usr, uint32_t sender_part, uint32_t receiver_part, const std::vector<uint8_t>& payload) {
        (void)receiver_part;
        if (_parser) {
            _parser->Parse(payload.data(), payload.size(), sender_usr, sender_part);
        }
    }
...
private:
    ...
    void handleRgaGeneric(uint32_t job_id, uint32_t usr, uint32_t part, ChainBuilder cb, 
                         ...
                         RgaConfig const& s_cfg, RgaConfig const& d_cfg, RgaConfig const& p_cfg) 
    {
        ...
        _runtime->dispatch([this, chain, job_id, usr, part, s, d, p]() mutable {
            RgaEngine::getInstance().submit(chain, [this, job_id, usr, part, s, d, p](uint32_t err_code) {
                auto guard = _builder.BuildRgaResponse(job_id, err_code);
                std::vector<uint8_t> payload(guard.data(), guard.data() + guard.size());
                if (_send_cb) {
                    _send_cb(SuOS::Config::Part::DISPLAY_RGA, usr, part, payload);
                }
            });
        });
    }
