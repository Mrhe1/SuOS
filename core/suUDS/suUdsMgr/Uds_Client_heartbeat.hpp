        ...
        public:
            ...
            void start() {
                ...
                _heartbeat_task = _runtime->scheduleAtFixedRate(100, _heartbeat_interval, [this]() {
                            _send_msg(_heartbeat_part, _router_id, _heartbeat_part, std::vector<uint8_t>());
                        }, false);
            ...
            void onMsg(uint32_t sender_usr, uint32_t sender_part,
                    uint32_t receiver_part, const std::vector<uint8_t>& sub_payload) {
                    if (sender_part == _heartbeat_part && sender_usr == _router_id && receiver_part == _heartbeat_part) {
